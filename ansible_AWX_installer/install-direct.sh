...
preflight_local() {
  ...
  # Internet: the GUEST (this host) pulls K3s + the awx-operator base + images.
  log_step "Preflight: internet reachability (from this host)"
  local crit_ok=0
  for u in https://get.k3s.io https://github.com; do
    if curl -fsS -m 10 -o /dev/null "$u" 2>/dev/null; then
      log_ok "reachable: ${u}"; crit_ok=$((crit_ok+1))
    else
      log_warn "NOT reachable: ${u}"
    fi
  done
  (( crit_ok >= 1 )) || blocker "Cannot reach get.k3s.io or github.com. K3s + awx-operator downloads will fail."
  if curl -fsS -m 10 -o /dev/null https://quay.io 2>/dev/null; then
