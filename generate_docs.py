...
def setup_styles(doc):
    """Configure document styles for headings and body text."""
...
def add_numbered_bold(doc, label, text):
    """Add a numbered list item with bold prefix."""
...
def add_severity_table(doc, headers, rows, col_widths=None):
    """Add a table with colored severity cells in column 0."""
    severity_colors = {
        "Critical": "FFCDD2",
        "High": "FFF9C4",
        "Medium": "C8E6C9",
...
def setup_header_footer(doc, header_text):
    ...
    for section in doc.sections:
        ...
        # Header (skip first page / title page)
...
def new_doc():
    """Create a new document with standard page setup and styles."""
    doc = Document()
    for section in doc.sections:
...
def create_strategy():
    ...
    add_title_page(
        ...
        version="1.0 DRAFT",
        owner=f"Global Security Office / Information Security",
    )
    ...
    add_para(doc, (
        ...
        "optimizes application performance, and meets security requirements. It also evaluates the "
        "transition from legacy hub-based deep packet inspection toward a Security Service Edge "
        "(SSE/SASE) architecture as a medium-term modernization path."
    ))
    ...
    add_numbered(doc, f"Position {C} for transition to a Security Service Edge (SSE/SASE) model as the long-term architecture.")
    ...
    add_table(doc,
        ...
        [
            ...
            ["Tier 0", "Internet Edge (SSE/Zscaler)",
             "Cloud-delivered SWG, CASB, ZTNA for internet-bound and SaaS traffic; threat inspection for web traffic",
    ...
    add_bold_para(doc, "Rationale: ", (
        ...
        "this at the site adds latency without additional security value."
    ))
    ...
    doc.add_heading("Tier 0: Internet Edge / SSE", level=2)
    add_para(doc, (
        "All internet-bound traffic from sites is already routed through the cloud proxy (e.g., Zscaler), "
        "providing Secure Web Gateway (SWG) capabilities."
    ))
    add_bullet(doc, "The SSE layer provides URL filtering, cloud DLP, malware scanning, and SSL inspection for internet-bound traffic.")
    add_bullet(doc, "Internet-bound traffic does NOT need to be inspected at the hub for IPS/DPI purposes\u2014it is handled by the SSE layer.")
    add_bullet(doc, "Future expansion of SSE capabilities (CASB, ZTNA) can progressively replace hub-based controls for SaaS application access and remote user connectivity.")

    doc.add_page_break()

    # -- SSE/SASE --
    doc.add_heading("Security Service Edge (SSE/SASE) Consideration", level=1)
    add_para(doc, (
        "The current tiered model is a pragmatic near-term approach that works with existing "
        "infrastructure. However, the industry is moving toward Security Service Edge (SSE) "
        "and Secure Access Service Edge (SASE) architectures, which converge networking and "
        "security into a cloud-delivered model."
    ))

    doc.add_heading("What SSE/SASE Offers", level=2)
    add_para(doc, (
        "SSE combines Secure Web Gateway (SWG), Cloud Access Security Broker (CASB), and "
        "Zero Trust Network Access (ZTNA) into a single cloud-delivered platform. SASE extends "
        f"this by integrating SD-WAN. For {C}, this would mean:"
    ))
    add_bullet(doc, "Reduced reliance on hub-based firewalls for deep inspection\u2014security inspection moves to the SSE cloud, closer to both the user and the application.")
    add_bullet(doc, "Simplified site architecture\u2014sites connect directly to the SSE cloud rather than backhauling through Equinix for inspection.")
    add_bullet(doc, "Consistent policy enforcement regardless of user location (warehouse, office, or remote).")
    add_bullet(doc, "Elimination of VPN concentrators for remote access, replaced by ZTNA.")

    doc.add_heading("Current Position and Recommendation", level=2)
    add_para(doc, f"{C} already uses a cloud proxy for SWG, which is the foundation of an SSE stack. The recommendation is:")
    add_numbered_bold(doc, "Near-term (0\u201312 months): ", "Implement the tiered enforcement model described in this document. This addresses the immediate alignment gap and can be deployed with current infrastructure.")
    add_numbered_bold(doc, "Medium-term (12\u201324 months): ", "Evaluate SASE vendors for ZTNA and CASB capabilities. Pilot with a subset of remote access and SaaS application traffic.")
    add_numbered_bold(doc, "Long-term (24\u201336 months): ", "Progressively shift inspection from hub firewalls to the SSE cloud. The hub\u2019s role transitions from full DPI to a routing and connectivity function.")
    add_bold_para(doc, "Key Message: ", (
        "The policies and standards in this document are written to be architecture-neutral where "
        "possible. The security requirements (what must be inspected) are separated from the "
        "enforcement location (where it is inspected). This means the standards remain valid as "
        "the enforcement location shifts from hub-based to cloud-delivered."
    ))
    ...
    add_table(doc,
        ["Security Control", "Site FW", "Hub FW (Equinix)", "Cloud (NSX/VPC)", "SSE (Proxy)", "TSG DC"],
        [
    ...
    add_numbered_bold(doc, "Site firewall (Tier 1): ", "Stateful ACL permits internet access via cloud proxy. Traffic is tunneled to the SSE, not via the hub.")
    add_numbered_bold(doc, "SSE / Cloud Proxy (Tier 0): ", "Full DPI, URL filtering, SSL inspection, malware scanning. This traffic does NOT traverse the hub.")
    add_bold_para(doc, "Total inspection points: ", "2 (stateful, SSE cloud). Hub is not involved.")
    ...
    add_table(doc,
        ...
        [
            ...
            ["SSE/SASE Strategy & Roadmap",
             "Detailed evaluation of SSE vendors, pilot plan, and migration roadmap from hub-based to cloud-delivered inspection.",
             "Planned", "Network Eng / InfoSec"],
        ],
    ...
    add_bullet(doc, "Validate that all sites are correctly routing through the hub for DPI/IPS and through the SSE for internet traffic.")

    doc.add_heading("Phase 4: Cloud & SSE Evaluation (Weeks 12\u201336)", level=2)
    add_bullet(doc, "Publish the Cloud Segmentation Standard covering GCVE and GCE.")
    add_bullet(doc, "Begin SSE/SASE vendor evaluation with a focus on extending cloud proxy capabilities (ZTNA, CASB).")
    add_bullet(doc, "Pilot ZTNA for a subset of remote access use cases to begin transitioning away from VPN concentrators.")
    add_bullet(doc, "Develop the detailed SSE/SASE roadmap for executive approval.")
    ...
    add_table(doc,
        ...
        [
            ...
            ["Network team capacity",
             "The network team may lack bandwidth to implement SASE in parallel with site rollouts.",
             "Phase the SSE/SASE evaluation into the medium-term. Focus on tiered model first."],
...
def create_netseg_standard():
    ...
    add_para(doc, (
        ...
        f"comprehensive framework covering site networks, hub/colocation facilities, data centers, "
    ...
    add_para(doc, (
        ...
        "between zones, and the security levels assigned to each zone. It provides the authoritative "
        "reference for how network segmentation is implemented across all environments."
    ))
    ...
    add_table(doc,
        ...
        [
            ...
            ["0", "SSE (Cloud Proxy)", "Web and SaaS traffic inspection, URL filtering, cloud DLP", "SWG, CASB, ZTNA (future)"],
        ],
    ...
    add_para(doc, f"Refer to the {C} Network Security Strategy (DOC-IT-IS-NSS-001) for the full enforcement point matrix and traffic flow walkthroughs.")
    ...
    add_para(doc, (
        ...
        "(ZTNA) solution as the SSE roadmap progresses."
    ))
    ...
    add_para(doc, (
        ...
        "change, or SSE/SASE migration milestone)."
    ))
    add_para(doc, (
...
def create_inspection_standard():
    ...
    add_para(doc, (
        ...
        "cloud-delivered inspection via Security Service Edge (SSE) platforms."
    ))
    ...
    add_bold_para(doc, "Use case: ", "Applied at the SSE layer for internet-bound traffic. Optional at the hub layer for internal encrypted traffic where compliance requirements mandate it.")
    ...
    add_bold_para(doc, "Decision: ", "WAF should be addressed in a separate standard or as an addendum. Listed here for completeness.")

    doc.add_page_break()

    # 4. Placement Requirements
    doc.add_heading("Inspection Placement Requirements", level=1)
    add_para(doc, "The following table defines the mandatory and optional inspection capabilities at each enforcement point. This is the authoritative reference to prevent duplication.")
    add_table(doc,
        ...
        [
            ...
            ["SSE / Cloud Proxy", "N/A", "MANDATORY (web)", "MANDATORY (web)", "MANDATORY"],
            ["TSG Datacenter", "MANDATORY", "MANDATORY", "MANDATORY", "OPTIONAL"],
        ],
        col_widths=[1.6, 1.0, 1.1, 1.0, 1.0],
    )
    add_para(doc, "NOT REQUIRED means the control is explicitly excluded at that point to avoid duplication. If a compliance requirement mandates additional inspection, a documented exception must be raised.")
    ...
    add_para(doc, (
        ...
        "specific signatures only where documented false positives have been identified."
    ))
    ...
    add_para(doc, "The following principles determine where inspection occurs:")
    add_bullet(doc, "Internet ingress traffic: Inspected by the SSE layer for web traffic, or by the hub/DC firewall for VPN or other non-web ingress.")
    ...
    add_bullet(doc, "Internet egress traffic: Inspected by the SSE layer. Does not transit the hub.")
    add_para(doc, "The key principle is single inspection per flow. Traffic should not be inspected by DPI/IPS at more than one point on its path unless there is a documented, approved reason.")
    ...
    # 8. SSE/SASE Transition
    doc.add_heading("SSE/SASE Transition Path", level=1)
    add_para(doc, f"As {C} evaluates and adopts SSE and SASE capabilities, the inspection model will evolve:")

    doc.add_heading("Current State", level=2)
    add_bullet(doc, "Cloud proxy provides SWG (Secure Web Gateway) for internet-bound traffic.")
    add_bullet(doc, "Hub firewalls provide DPI/IPS for site-to-cloud traffic.")
    add_bullet(doc, "NSX provides micro-segmentation for GCVE workloads.")

    doc.add_heading("Target State (Medium-Term)", level=2)
    add_bullet(doc, "Cloud proxy expanded to include ZTNA (Zero Trust Network Access) replacing traditional VPN for remote access.")
    add_bullet(doc, "CASB protecting SaaS applications.")
    add_bullet(doc, "Hub firewalls remain for site-to-cloud DPI/IPS but with simplified rulesets as cloud-native controls mature.")

    doc.add_heading("Target State (Long-Term)", level=2)
    add_bullet(doc, "Full SASE architecture where site traffic is routed directly to the SSE cloud for inspection, bypassing the hub for most traffic types.")
    add_bullet(doc, "Hub firewalls reduced to a connectivity/routing function with minimal inspection.")
    add_bullet(doc, "All inspection capabilities (DPI, IPS, anti-malware, DLP, CASB) delivered as cloud services.")
    add_bullet(doc, "This standard will be updated to reflect the new enforcement points when the SASE transition reaches each milestone.")

    # 9. Compliance
    doc.add_heading("Compliance and Review", level=1)
    add_para(doc, (
        "This standard must be reviewed annually or when significant changes occur to the "
        "inspection infrastructure (new firewall platforms, SSE migration milestones, new "
