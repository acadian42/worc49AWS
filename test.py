...
def cspm_iom_report(request: Request, config: dict | None = None) -> Response:
    ...
    try:
        ...
        findings = []
        api_debug = []   # what the API actually returned, page by page
        ...
        for sev in severities:
            ...
            while True:
                ...
                if code != 200:
                    # Surface the REAL error in the body (no errors=[dict]!).
                    return Response(
                        code=200,
                        body={
                            "ok": False,
                            "stage": "GetConfigurationDetections",
                            "severity": sev,
                ...
                resources = rbody.get("resources") or []
                api_debug.append({
                    "severity": sev,
                    "page": page,
                    "resource_count": len(resources),
                    "first_resource_keys": list(resources[0].keys()) if resources else [],
                })
...
def _normalize(item, sev_fallback):
    ...
    Inspect body.findings (or body.debug) after a test run and tweak these keys."""
    nested = None
