...
# ---- Buckets / paths ---------------------------------------
SOURCE_BUCKET=""
SOURCE_PREFIX=""    # stripped from destination keys
JOURNAL_ROOT=""                  # date tree lives under here
DEST_BUCKET=""
...
# AWN files are the migrate set now.
FILTER_REGEX='awn-optimized'
...
MODE="${MODE:-validate}"
SMOKE_N="${SMOKE_N:-3}"            # sample files to integrity-check in validate mode
...
# State is namespaced per date-range AND suffixed _awn so this run
# never collides with the earlier customer-log run's state/joblog.
# NOTE: do NOT copy objects.txt over from the old customer state dir --
# that cache still lists the customer keys you have since deleted.
WORK_DIR="$HOME/migration-state/${START_DATE}_to_${END_DATE}_awn"
mkdir -p "$WORK_DIR"
...
# ---- Phase 1: selection + reconciliation -------------------
# Customer* is migrated and deleted, so the window should be 100%
# awn-optimized: expect MATCH == TOTAL, leftovers == 0, remnants == 0.
report_selection() {
  grep -E "$FILTER_REGEX" "$OBJECT_LIST" > "$FILTERED_LIST" || true
  local total match skip cust_left
  total=$(wc -l < "$OBJECT_LIST")
  match=$(wc -l < "$FILTERED_LIST")
  skip=$(( total - match ))
  cust_left=$(grep -c 'customer' "$OBJECT_LIST" || true); cust_left=${cust_left:-0}

  echo
  echo "================= SELECTION / DATE SUMMARY ================="
  echo "Date window (inclusive)   : $START_DATE  ->  $END_DATE"
  echo "Objects in window (all)   : $total"
  echo "WILL MIGRATE (awn-opt)    : $match"
  echo "LEFT OVER (not selected)  : $skip       <- expect 0"
  echo "customer* remnants        : $cust_left       <- expect 0 (already deleted)"
  echo "============================================================"
  if [[ "$total" -eq 0 ]]; then
    echo "!! WARNING: listing came back EMPTY. Wrong window/prefix, or source already drained."
  elif [[ "$skip" -ne 0 ]]; then
    echo "!! WARNING: $skip object(s) in the window are NOT awn-optimized."
    echo "   They will NOT be migrated. Inspect these BEFORE running live:"
    grep -vE "$FILTER_REGEX" "$OBJECT_LIST" | head -n 10 | sed 's/^/     /'
  else
    echo "OK: selection covers 100% of the window (everything left is awn-optimized)."
  fi
  echo
